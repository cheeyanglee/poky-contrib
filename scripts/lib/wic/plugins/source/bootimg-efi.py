...
class BootimgEFIPlugin(SourcePlugin):
    ...
    def do_configure_grubefi(cls, hdddir, creator, cr_workdir, source_params):
        ...
        if not custom_cfg:
            ...
            multi_boot_options = get_bitbake_var("MULTI_BOOT_OPTIONS") if get_bitbake_var("MULTI_BOOT_OPTIONS") else ""
            for boot_option in multi_boot_options.split(';'):
                ...
                grubefi_conf += "linux /%s %s rootwait %s %s\n" \
                    % (kernel, label_conf, bootloader.append, boot_option)
    ...
    def do_configure_systemdboot(cls, hdddir, creator, cr_workdir, source_params):
        ...
        if not custom_cfg:
            ...
            conf_count = 0
            for boot_option in multi_boot_options.split(';'):
                conf_title = "%s-%s" % ( title, conf_count) if boot_option else title
                ...
                boot_conf += "options %s %s %s\n" % \
                                 (label_conf, bootloader.append, boot_option)
